...
async function runModel()
{
	...
	{
		const margin = 5
		const imgRect = img.crop(rect.xMin - margin, rect.yMin - margin, rect.xMax + margin, rect.yMax + margin).stretch(32, 32)
	...
	for (const rect of rects)
	{
		...
		{
			ctx.fillStyle = "#080"
			ctx.font = "Verdana 4px"
			ctx.textAlign = "left"
			ctx.textBaseline = "middle"
			ctx.fillText(rect.kind, rect.xMax + 6, rect.yMin - 6)
			ctx.textAlign = "right"
			ctx.fillText(rect.kind, rect.xMin - 6, rect.yMin - 6)
		})
	}
